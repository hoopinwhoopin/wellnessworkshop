...
export default function Navlist({callback}) {
  ...
  return (
    ...
    <StyledButton onClick={()=>{callback(false)}} href="/#Gallery" className='font-semibold text-xl max-md:text-3xl w-fit'>Gallery</StyledButton>
    <StyledButton  onMouseOver={(e)=>{setAnchorEl(e.currentTarget)}} className='font-semibold text-xl max-md:text-3xl h-full w-fit justify-between'><div className='flex flex-row whitespace-break-spaces items-center' style={{color:openMenu? "#C94277":"black"}}>Packages   <Icon openMenu={openMenu}/></div></StyledButton>
    <Menu
      id="simple-menu"
      anchorEl={anchorEl}
      open={openMenu}
